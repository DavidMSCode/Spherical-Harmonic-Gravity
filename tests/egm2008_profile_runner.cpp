...
int main(int argc, char* argv[]) {
    ...
    // Warm-up load/path validation
    {
        ...
        (void)SHG::g_EGM2008(p.r_m, phi, lambda, degree, coefficient_path);
    ...
    for (int i = 0; i < iterations; ++i) {
        for (const auto& p : points) {
            const double phi = deg2rad(p.phi_deg);
            const double lambda = deg2rad(p.lambda_deg);
            const auto g = SHG::g_EGM2008(p.r_m, phi, lambda, degree, coefficient_path);
            sink += std::sqrt(g[0] * g[0] + g[1] * g[1] + g[2] * g[2]);
        }
    }
    ...
    const auto g_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
    const auto u_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();
    const double calls = static_cast<double>(iterations * static_cast<int>(points.size()));

    std::cout << std::fixed << std::setprecision(3);
    std::cout << "degree=" << degree << ", iterations=" << iterations
              << ", points_per_iter=" << points.size() << std::endl;
    std::cout << "g_EGM2008 total_ms=" << g_ms << ", per_call_ms=" << (g_ms / calls) << std::endl;
    std::cout << "U_EGM2008 total_ms=" << u_ms << ", per_call_ms=" << (u_ms / calls) << std::endl;
