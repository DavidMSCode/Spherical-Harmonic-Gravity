...
T g(double r, double phi, double lambda, int l_max, int m_max, const std::vector<std::vector<double>> &C, const std::vector<std::vector<double>> &S, double a, double GM)
{
    ...
    if (is_workspace_enabled()) {
        ...
        static thread_local std::vector<double> mTan_ws;
        static thread_local std::vector<double> sinL_ws;
        static thread_local std::vector<double> cosL_ws;
        Plm_bar_flat_inplace(l_max + 1, m_max + 1, phi, P_ws_flat);
        recursive_tangent_inplace(m_max, phi, mTan_ws);
        recursive_sine_cosine_inplace(m_max, lambda, sinL_ws, cosL_ws);

        const int cols = (m_max + 1) + 1;
        ...
        for (int l = 2; l <= l_max; ++l)
        {
            ...
            for (int m = 0; m <= l; ++m)
            {
                ...
                double P_lmp1 = P_ws_flat[p_idx(l, m + 1)];
                double cos_m_lambda = cosL_ws[m];
                double sin_m_lambda = sinL_ws[m];
                double mtan_phi = mTan_ws[m];
